...
var actor_stats: Resource setget set_actor_stats
var _ignore_value_change := false


func _ready():
	run_speed_slider.connect("value_changed", self, "_on_value_changed")
	strength_spinbox.connect("value_changed", self, "_on_value_changed")
	endurance_spinbox.connect("value_changed", self, "_on_value_changed")
	intelligence_spinbox.connect("value_changed", self, "_on_value_changed")
	
	
func set_actor_stats(new_actor_stats: Resource) -> void:
	if actor_stats != new_actor_stats:
		new_actor_stats.connect("health_changed", self, "update_health_label")
		new_actor_stats.connect("runtime_collected", self, "update_reward_label")
		if new_actor_stats.has_signal("global_position_changed"):
			new_actor_stats.connect("global_position_changed", self, "update_player_position")
